...
#include "tnt_array2d.h"
   ...
   /** LU Decomposition.
   <P>
   For an m-by-n Array2D A with m >= n, the LU decomposition is an m-by-n
   unit lower triangular Array2D L, an n-by-n upper triangular Array2D U,
   and a permutation Array1D piv of length m so that A(piv,:) = L*U.
   If m < n, then L is m-by-m and U is m-by-n.
   <P>
   The LU decompostion with pivoting always exists, even if the Array2D is
...
class LU
{
   ...
   */
   ...
   /** Return pivot permutation Array1D
   @return     piv
   */
   ...
   */
   ...
   @param  b   a Array1D (Array1D> of length equal to the first dimension
   						of A.
   @return x a Array1D (Array1D> so that L*U*x = b(piv), if B is nonconformant,
   ...
   */
   ...
   void mprove(Array1D<Real> &x, const Array2D<Real> &A, const Array1D<Real> &b)
   {
	   if (x.dim()!=A.dim2() || b.dim()!=A.dim1())
		   return;
	   Array1D<Real> bplus = matmult(A, x);
	   for (int i=0; i<b.dim(); i++)
		   bplus[i] -= b[i];
	   Array1D<Real> aplus = solve(bplus);
