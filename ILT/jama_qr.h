...
<p>
   ...
   for an m-by-n Array2D A with m >= n, the QR decomposition is an m-by-n
   orthogonal Array2D Q and an n-by-n upper triangular Array2D R so that
   A = Q*R.
<P>
   The QR decompostion always exists, even if the Array2D does not have
...
/**
	...
	@param A rectangular (m>=n) Array2D.
*/
	QR(const Array2D<Real> &A)		/* constructor */
	{
      ...
      for (k = 0; k < n; k++) {
         ...
         Real nrm = 0;
         for (i = k; i < m; i++) {
            nrm = my_hypot(nrm,QR_[i][k]);
         }

         if (nrm != 0.0) {
            // Form k-th Householder Array1D.
            if (QR_[k][k] < 0) {
               nrm = -nrm;
            }
            for (i = k; i < m; i++) {
...
/**
	Flag to denote the Array2D is of full rank.

	@return 1 if Array2D is full rank, 0 otherwise.
*/
   ...
   @returns lower trapezoidal Array2D whose columns define the reflections
   */
   ...
   */
   ...
   @param B     m-length array (Array1D).
   @return x    n-length array (Array1D) that minimizes the two norm of Q*R*X-B.
   		If B is non-conformant, or if QR.isFullRank() is false,
						the routine returns a null (0-length) Array1D.
   */
   ...
   Array1D<Real> solve(const Array1D<Real> &b) const
   {
	  ...
	  if ( !isFullRank() )		/* Array2D is rank deficient */
	  {
	  	return Array1D<Real>();
	  }
