...
class SVD 
{
  ...
  public:
   ...
   SVD(const Array2D<Real> &Arg)
   {
      ...
      bool wantu = 1;  					/* boolean */
      bool wantv = 1;  					/* boolean */
	  int i=0, j=0, k=0;
   ...
   */
   ...
   int rank() 
   {
      Real tol = max(m,n) * norm2() * pow(2.0,-52);
      int r = 0;
   ...
   {
	   Array1D<Real> tmp(n);
	   Real tol = max(m, n) * pow(2.0,-52) * norm2();

	   // Calculate s * transpose(U) * b
	   // zeros terms that are close to singular.
	   for (int j=0; j<n; j++) {
		   tmp[j] = Real(0);
		   if (s[j] > tol) {
			   for (int i=0; i<m; i++)
	   ...
	   // Multiply V by the above result to get x
	   Array1D<Real> x(n);
	   for (int j=0; j<n; j++) {
		   x[j] = 0.0;
		   for (int i=0; i<n; i++)
